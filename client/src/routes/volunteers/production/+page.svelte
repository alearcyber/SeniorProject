...
<div class="mb-10 flex justify-center" >
    <form class="flex flex-col space-y-6" style="width:800px" action='/'>
        ...
        <div>
            <Label class="space-y-2">
                <span>Date</span>
                <Input type="text" name="performance_date"  bind:value={values[i].date} placeholder="MM/DD/YYYY"  required />
                <span>Time</span>
                <Input type="text" name="performance_time"  bind:value={values[i].time} placeholder="X:XX p.m." required />
                <span>Default Seat Price</span>
                <Input type="text" name="seat_price"  bind:value={values[i].seats} placeholder="$" required />
                ...
                {#if venue_input == "Civic Center Playhouse"}
                    ...
                    <Input type="text" name="orch_price"  bind:value={values[i].sections} placeholder="$" required />
                    <span>Balcony Price</span>
                    <Input type="text" name="balcony_price"  bind:value={values[i].sections} placeholder="$" required />
                    <span>Loge Price</span>
                    <Input type="text" name="loge_price"  bind:value={values[i].sections} placeholder="$" required />
                    <span>Box Price</span>
                    <Input type="text" name="box_price"  bind:value={values[i].sections} placeholder="$" required />
                    <span>Handicap Price</span>
                    <Input type="text" name="handicap_price"  bind:value={values[i].sections} placeholder="$" required />

                {:else if venue_input == "Civic Center Concert Hall"}
                    <span>Orchestra Price</span>
                    <Input type="text" name="orch_price"  bind:value={values[i].sections} placeholder="$" required />
                    <span>Pit Price</span>
                    <Input type="text" name="pit_price"  bind:value={values[i].sections} placeholder="$" required />
                    <span>Lower Balcony Price</span>
                    <Input type="text" name="lower_balcony_price"  bind:value={values[i].sections} placeholder="$" required />
                    <span>Upper Balcony Price</span>
                    <Input type="text" name="upper_balcony_price"  bind:value={values[i].sections} placeholder="$" required />
                    <span>Loge Price</span>
                    <Input type="text" name="loge_price"  bind:value={values[i].sections} placeholder="$" required />
                    <span>Handicap Price</span>
                    <Input type="text" name="handicap_price"  bind:value={values[i].sections} placeholder="$" required />
